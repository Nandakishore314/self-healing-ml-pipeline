...
def test_post_predict_credit_risk_scoring():
    ...
    train_payload = {"model_type": "rf", "data_source": "data/historical_data.csv"}

    try:
        train_response = requests.post(train_url, json=train_payload, timeout=TIMEOUT)
        assert train_response.status_code == 200, (
            f"Training failed with status code {train_response.status_code}"
        )
        ...
        predict_response = requests.post(
            predict_url, json=applicant_features, timeout=TIMEOUT
        )
        assert predict_response.status_code == 200, (
            f"Predict failed with status code {predict_response.status_code}"
        )
        predict_json = predict_response.json()
        # Validate response keys and types
        assert "default_prediction" in predict_json and isinstance(
            predict_json["default_prediction"], int
        )
        assert "default_probability" in predict_json and isinstance(
            predict_json["default_probability"], float
        )
        assert "risk_assessment" in predict_json and isinstance(
            predict_json["risk_assessment"], str
        )
        ...
        invalid_features["AMT_INCOME_TOTAL"] = (
            "invalid_float"  # cause server-side scoring error
        )

        predict_500_response = requests.post(
            predict_url, json=invalid_features, timeout=TIMEOUT
        )
        assert predict_500_response.status_code in (400, 422, 500), (
            f"Expected error status 400 or 422 or 500, got {predict_500_response.status_code}"
        )
